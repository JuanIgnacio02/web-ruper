...
export default function AnimalSelector({ activeAnimal, onFilter }: Props) {
  ...
  // Sync panel expansion with active filter from filter bar
  useEffect(() => {
      ...
      <div className="as-panels-wrap flex flex-col md:flex-row min-h-[540px]">
        {PANELS.map(panel => {
          ...
          return (
            ...
            >
              {/* Overlay */}
              <div className={`absolute inset-0 transition-all duration-500 ${isActive ? 'bg-black/50' : 'bg-black/65 saturate-0'}`} />
              ...
              <div className={`relative z-10 h-full flex flex-col justify-end p-6 md:p-10 transition-all duration-300`}>
                ...
                {!isActive && (
                  <div className="flex items-center gap-3 md:flex-col md:gap-2">
                    ...
                    <h3 className="text-white font-black text-[1.4rem] md:text-[1.1rem] tracking-wider">{panel.label}</h3>
                  </div>
                )}
                ...
                {isActive && (
                  <div onClick={e => e.stopPropagation()} className="w-full max-w-[400px]">
                    <span className="text-5xl mb-3 block">{panel.emoji}</span>
                    <div className="flex items-center gap-3 mb-6">
                      <div className="w-6 h-6 rounded-full border-2 border-[var(--primary)] flex items-center justify-center flex-shrink-0">
                        <i className="fas fa-check text-[var(--primary)] text-[.6rem]" />
                      </div>
                      <h3 className="text-white font-black text-[2rem] tracking-wider">
                        <span className="text-[var(--primary)] text-[.9rem] font-black mr-1">RUPER</span>
                        {panel.label}
                      </h3>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 mb-5">
                      {panel.filters.map(f => (
                        <div key={f.key}>
                          <label className="text-white/60 text-[.72rem] font-semibold uppercase tracking-wide block mb-1.5">{f.label}:</label>
                          <div className="relative">
                            <select
                              value={selects[panel.animal][f.key]}
                              onChange={e => setSelect(panel.animal, f.key, e.target.value)}
                              className="w-full bg-white/15 backdrop-blur-sm border border-white/25 text-white rounded-xl px-4 py-3 text-[.88rem] font-medium appearance-none focus:outline-none focus:border-[var(--primary)] transition-colors"
                            >
                    ...
                    <button
                      ...
                      className="w-full bg-[var(--primary)] hover:bg-[var(--primary-dark)] text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2.5 transition-all hover:-translate-y-0.5 text-[.96rem]"
                    >
